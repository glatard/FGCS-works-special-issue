...
%% Text of abstract
  ...
  components and interactions in 6 architectures. We use these
  architectures to classify 18 existing systems and we compare these
  architectures using metrics measuring integration effort,
  ...
  model is the most scalable. Meta-workflows are only available in
  nested workflows and workflow import.
\end{abstract}
...
WS-PGRADE~\cite{Kacsuk2012}, etc. Such workflow engines usually
describe applications in a specific language that offers parallel data
and control flow constructs, visual edition features, links with
domain-specific tool repositories, provenance recording, among other
features (see review in~\cite{deelman2009workflows}).
...
The architectures are diagrammed in Figure~\ref{fig:architectures}
...
interactions among these components are represented with arrows.
\emph{Abstract} interactions are specific types of interactions that
may be implemented by various different software interactions. They
are represented with dotted arrows and labeled with \texttt{*}. An
...
interactions involved in the architectures, details architectures and
illustrates them using examples of real
...
security reasons. This is the model adopted in LONI Pipeline
Environment~\cite{dinov2009efficient}, in the Distributed application
runtime environment (DARE~\cite{maddineni2012distributed}), and in the
Catania Science Gateway Framework~\cite{Ardizzone2012} which is used
for instance in the DECIDE gateway for 
neuroimaging~\cite{ardizzone2012decide} (see specific documentation on
...
in~\cite{wu2010accelerating}, the WS-PGRADE/gUSE
framework~\cite{Kacsuk2012} and the numerous science gateway instances
that rely on WS-PGRADE/gUSE, for instance
e-BioInfra~\cite{shahand2015data}, MoSGrid~\cite{krüger2014mosgrid},
and many others~\cite{kacsuk2014science}. The
integration between the HubZero science gateway and Pegasus workflow
engine performed in~\cite{CPE:CPE3257} also use a service architecture
where the \texttt{d} interaction is implemented as a set of calls to
the Pegasus command-line tools (e.g. pegasus-status,
pegasus-statistics, etc). Figure~\ref{fig:vip-architecture} shows the
architecture of the Virtual Imaging Platform, where the MOTEUR
workflow engine~\cite{GLAT-08i} is invoked as a service at step
...
\caption{Architecture of the CBRAIN-FSL integration (sub-tasking). 1:
  ...
  storage server(s). 4-5: Execution server starts FSL tool (pipeline)
  via task scheduler. 6: FSL pipeline submits sub-tasks execution
  server through fsl\_sub. 7-8: Execution server starts sub-tasks
  through task scheduler. 9.  Execution server transfers results to
  storage server(s).}
\label{fig:cbrain-fsl-architecture}
\end{figure}
The CBRAIN-FSL integration allows to leverage FSL pipelines that are
written in a very low-level workflow language (Linux executables and
script) that submits tasks uniformly through a specific tool called
fsl\_sub. It is represented in
Figure~\ref{fig:cbrain-fsl-architecture}.  On this Figure, the science
...
and \texttt{9}. Interaction \texttt{e} is implemented by step
\texttt{6} through a modified version of the fsl\_sub
script\footnote{See modified fsl\_sub script available at
  \url{https://github.com/aces/cbrain-plugins-neuro}}.

\begin{figure}
\def\svgwidth{\columnwidth}
\input{figures/CBRAIN-PSOM.pdf_tex}
\caption{Architecture of the CBRAIN-PSOM integration (sub-tasking,
  Figure extracted from~\cite{GLAT-16}). 1: User sends data and
  workflow execution request to storage server(s) and CBRAIN
  portal. 2: CBRAIN portal sends execution request to execution server
  on cluster. 3: Execution server transfers data from storage
  server(s). 4-5: Execution server starts PSOM master via task
...
\end{figure}
The CBRAIN-PSOM integration~\cite{GLAT-16} is illustrated on
...
the a master coordinates workflow execution by submitting workers and
...
used in the SHIWA Simulation Platform for nested workflow execution with
...
\item Fine-grained debugging (\texttt{O$_2$}): availability of
  fine-grained debugging information about workflow tasks. When such
  information is available, workflow is seen as a white
  ...
  such information is easy to access, \texttt{O$_2$}=1 otherwise.
\end{itemize}
...
version in a workflow service is transparent since the only goal of
this component is to wrap the engine.  New workflows are added in the
science gateway or in the workflow engine through interactions
\texttt{a$_1$} and \texttt{a$_2$} (\texttt{E$_3$}=2). Adding a new
type of infrastructure requires updates in interactions \texttt{b},
\texttt{$c_1$} and \texttt{$c_2$} (\texttt{E$_4$}=3).
...
\textbf{Total} (scalability issues)  & \cellcolor[HTML]{99BB99}\textbf{3}
                                     ...
                                     & \cellcolor[HTML]{99DD99}\textbf{2}
                                     & \cellcolor[HTML]{99DD99}\textbf{2}\\
\multicolumn{7}{c}{\cellcolor[HTML]{EEEEEE}\textbf{Specific features}}\\
