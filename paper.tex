...
\begin{abstract}
  ...
  child process of another engine. In \emph{workflow import}, the
  ...
  import. These results provide insights for science gateway
...
main components and interactions. It is informed \note{vladimir}{is this a good word for this context, 'informed'? maybe 'based on'?} by our experience in
...
Section~\ref{sec:architectures}, we describe six architectures \note{vladimir}{why is it exactly six? are these architectures the ones that happened to be implemented during various projects, or is it a full set of all possibilities (I believe not)? I think, it must be stated clearly why the focus is given to these particular ones - we are reviewing and evaluating the approaches that have been used already in our projects} within
...
extensibility, scalability and other specific features \note{vladimir}{can we explain why exactly these metrics are used but not anything else? I think, we need to justify that these ones are especially important for the SG case; it would be nice to refer to some related research on software architecture metrics, Google gives about 2 million pages on this, where you can find metrics like cohesion, depth of scenario and so on}. We finally
...
and WS-PGRADE~\cite{Kacsuk2012} \note{vladimir}{As a past developer of it, I would suggest to refer to WS-VLAM~\cite{wsvlam} as well, despite it is not very famous, but it played an important role for Amsterdam e-science workflows}. Such workflow engines usually
...
\item Workflow integration: consists in adding a new workflow to the
  ...
  the infrastructure, which may require administrative privileges \note{vladimir}{or bundled in packages considering all needed dependencies than can be deployed on the fly during the workflow execution};
...
workflow engine directly to the user. \answerednote{Sil}{about c1: should it
  ...
  this architecture. I modified a bit the text, is it clearer now?}
...
format. \note{vladimir}{Are we talking only about importing workflows from other wf languages? I would suggest to modify the title of the section to 'Workflow conversion and import' then, as some gateways use the term 'import' to import their own native workflows, like WS-PGRADE, and using import only in the context of converting workflows can be confusing} Workflow import is usually an offline process that is not
...
architecture. 
\note{vladimir}{I believe, proper error handling through the different levels of wf engine/gateway must be especially mentioned even if not especially covered. One of serious issues we faced while working with gateways at the AMC, in particular with gUse/WS-PGRADE, was that when it failed sometimes it was completely not clear what was the problem exactly. So we might consider discussing not only the number of software components but how explicit and clear interactions are between them; how transparently we can monitor the execution through all the levels and thus detect and prevent possible malfunction}
...
\item Multiple engine instances (\texttt{S$_1$}): measures the potential of having
  ...
  for the science gateway to means \note{vladimir}{is this the right word here, 'to means'?} that adding a new engine instance is a
...
\item Meta-workflow (\texttt{O$_1$}): measures the ability to describe
  ...
  components. \texttt{O$_1$}=0 means that meta-workflows are enabled by the architecture, and
  \texttt{O$_1$}=1 otherwise.
  \note{si}{why here we do not have the "possible to implement" situation?}
  \note{vladimir}{I agree with Silvia, we should have the intermediate values as well, since meta-workflows might be straightforwardly supported by the gateway, or they are still possible to implement but only with some workarounds. Like it was with SHIWA: first 6 months coarsed-grained interoperability looked more like a workaround till it was better integrated and supported by WS-PGRADE itself}
...
\item Fine-grained debugging (\texttt{O$_2$}): availability of
  ...
  \texttt{O$_2$}=0 means that such information is easy to access, and
  \texttt{O$_2$}=1 otherwise. \note{vladimir}{otherwise - does it mean the fine-grained info is available but hard to access or not available it all? the latter happens as well}
\end{itemize}
\texttt{O$_1$} and \texttt{O$_2$} are summed to obtain a total number of missing specific
features in the architecture.

\note{sil}{since we are dreaming, it seems to me that keeping provenance properly is also something to consider. the architecture can hide some details, and make it nearly impossible to keep provenance without hard coded hacks across the layers/components - i think}
...
\note{sil}{why not? i agree that S=2, but i do not agree that it would be impossible. you say how below}
(\texttt{S$_1$}=2). 
Specific IT setups such as load-balancing between
web server instances might be used to create new gateway instances, distributing the users, but would not solve the problem for the workload of a single workflow or user.
Distributed engines are not available by default
(\texttt{S$_2$}=1). 
The scheduling of tasks on the infrastructure is
...
\note{sil}{so same as the others = "easy"? i think this requires more explanation. maybe we should introduce a score for "dont know"?}

\paragraph{Specific features} Meta-workflows are not supported by default
(\texttt{O$_1$}=1).  
\note{sil}{why not? if the engine is ws-pgrade it does. i really think that meta-workflows (and nested workflows) is a property exclusively of the engine. you can see that in fig 2.V interaction *6}
Debugging is not an issue since the science
...
new engine instance, however, requires manual intervention to declare
the new instance in the science gateway
(\texttt{S$_1$}=1). Consequently, elastic engines are difficult to
implement because they require a mechanism to update the science
gateway configuration when a new engine instance is
...
specific abilities (\texttt{S$_2$}=1).  \answerednote{sil}{is this score for
  ...
  we should not say s_2=1 for the architecture}{Tristan}{Removed VIP example. Score definition still has to be modified according to the discussion in @SilviaOlabarriaga's PR (\#20).} The scheduling of
tasks on the infrastructure is as complex as in any other architecture
since the workflow engine might implement any kind of scheduling
policy (\texttt{S$_3$}=0).  \note{sil}{see comment in section 3.1}

\paragraph{Specific features} Meta-workflows are not supported by default
(\texttt{O$_1$}=1). 
\note{sil}{see comment in section 3.1}
Fine-grained debugging information is usually easy
to obtain since the workflow service provides direct access to the
engine (\texttt{O$_2$}=0).
\note{sil}{this is really not the case. the fact that the wf service is a separate component that hides information is a big problem for debugging at the science gateway level. 
we had a paper about this:
...
(\texttt{R$_2$}=1).
\note{vladimir}{there is a large piece of text in Scalability about reliability of the workflow engine task which on my opinion suits Robustness better}
\note{vladimir}{in addition, the claim of very good robustness of task encapsulation seems questionable to me as in this model you delegate the critical task of controlling workflow execution to, let's say, some 'random' node on the grid that you have no control of, not sure about its performance, connectivity and reliability and so on; however, considering the models and dependencies presented in fig.2 this evaluation of robustness metric may be correct}
...
which increases its chances of failure.\note{vladimir}{isn't this about robustness?} In addition, task parameters,
...
(\texttt{O$_1$}=1).  
\note{sil}{see comment in section 3.1}
Obtaining fine-grained information about workflow
...
\note{sil}{mm, i think the case here is the same as for service. or actually i would give this one a better score, because there are less components to cross}
...
(\texttt{O$_1$=1}).  
\note{sil}{see comment in section 3.1}
Accessing debugging information is not likely to
be an issue since the workflow pool could implement specific functions
for that (\texttt{O$_2$=0}).
\note{sil}{i think the case here is the same as for service. or actually i would give this one a worse score, because there is one additional component - the pool - to cross}
...
\texttt{d} (twice).
\note{vladimir}{and if we have a meta-workflow with 3 and more sub-workflows the score will be even higher, is it a fair calculation for an architecture? we did not increase the scores for possible meta-workflows supported by wf engines in other cases}
...
(\texttt{O$_1$=0}). 
\note{sil}{i dont understand this about meta-workflows. translation does not imply composition, right? if the science gateway is allowed to implement workflow composition, then this is probably the case in other architectures as well?}
Debugging information is accessible as in the
service invocation architecture (\texttt{O$_2$=0}).
\note{sil}{see notes 3.2}
...
gateways that do not focus on a particular engine.  However, task encapsulation, workflow import (with
...
invocation) are the only architectures that support meta-workflows. \note{vladimir}{As mentioned earlier by Silvia, this might not be exactly the case, depending on the point of view} Besides,
