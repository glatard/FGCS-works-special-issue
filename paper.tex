...
\newcommand{\closednote}[4]{\pdfmargincomment[color=gray,author=#1-#3,open=false]{#1: #2 \textCR \textCR #3: #4}}
\newcommand{\answerednote}[4]{\pdfmargincomment[color=green,author=#1-#3,open=true]{#1: #2 \textCR \textCR #3: #4}}
\newcommand{\closedanswerednote}[6]{\pdfmargincomment[color=gray,author=#1-#3-#5,open=true]{#1: #2 \textCR \textCR #3: #4 \textCR \textCR #5: #6}}
...
\begin{abstract}
  ...
  the CBRAIN and VIP platforms, and on our past involvement in the
  SHIWA project. We abstract software components and interactions in 6
  architectures that we use to classify 18 systems. We compare these
  architectures using metrics measuring global complexity, robustness,
  extensibility, scalability and other specific features. Results show
  the pros and cons of the different architectures: tight integration
  and sub-tasking are the simplest architectures and should therefore
  be the easiest to implement and the most robust ones, most
  ...
  nested workflows and workflow import. Our analysis provides insights
...
computing infrastructures, in particular cluster schedulers, to execute tasks at
...
domain-specific pipelines.\note{Marc-e}{To remain consistent with above descriptions} Some workflow engines, usually e-Science
...
instances were also built independently from such specialized framework, using
regular web development tools and libraries to access distributed
systems. CBRAIN and VIP are two such science gateways, from which we
gathered experience about the architectures described in this paper.
...
centers in Canada, a cluster in Germany, and on numerous local research
servers. CBRAIN is a task-centric platform where various workflow engines
can be deployed on the infrastructure as any other task would. As February 2016,
CBRAIN has served over 420 users across 50 cities in 22 countries and has a
national computing allocation of 660 CPU years per annum.
...
\subsection{Sub-tasking} \note{Marc-e}{May I propose another name here? Either
"Task-centric" or "Task Oriented". My reasoning is that the WE is just another
task, as well as the sub-tasks themselves.}
...
peculiar execution model is well supported by the sub-tasking
architecture.
...
\caption{CBRAIN architecture for workflow engine integration
  ...
  \texttt{b} in Figure~\ref{fig:architectures} is implemented by steps
...
\texttt{14}). \note{Marc-e}{Maybe easier to follow if the following would be in fig
 legend instead of here, as for 2 previous figures} In Figure~\ref{fig:shiwa-pool-architecture},
...
evaluate the architectures: global complexity, execution complexity,
...
\paragraph{Global complexity} Service invocation requires a workflow service
...
\paragraph{Global complexity} The sub-task architecture requires only 2
...
in this architechture shares the same mechanism as version updates of existing
tasks or tools, which requires an update on the infrastructure (\texttt{E$_2$}=1).  New
...
infrastructure requires to update interactions \texttt{a$_1$}, \texttt{b} and
\texttt{c} in the science gateway (\texttt{E$_4$}=3).
...
\paragraph{Global complexity} Setting up a nested workflow
...
\texttt{a$_1$}, \texttt{a$_2$}, \texttt{b}, \texttt{c$_1$} (twice: once for each infrastructure),
\texttt{c$_2$} (twice: once for each workflow engine and infrastructure) and
\texttt{d} (twice: once for each workflow engine).
\closedanswerednote{Rafael}{I like the idea of what has been discussed in
  ...
  github issue \#8.} \note{Marc-e}{I understand both points. However, complexity is
  addressed in Robustness as well... I liked the idea of development effort. It's a tough call.}
...
update infrastructure (sub-tasking), science gateway (tight
...
which may be interesting in some cases. Also, sub-tasking and nested
...
favor some of them. Overall, sub-tasking, and workflow import (with
...
which would favor sub-tasking. Similarly, adding a new type of engine
