...
Octave and Matlab~\cite{bellec2012pipeline}), RPPL~\cite{1174106} and
PMP (Poor Man's Pipeline~\cite{Ad-DabbaghY2006}), SPM (Statistical
...
tracking, \closednote{Naj}{should we not mention PMP which is used for
  CIVET?}{Tristan}{Done, as discussed.}  redundancy detection across
analyses to avoid re-computation, and so on. A wide-array of
workflows have been implemented using these pipelines and are now
shared across neuroimaging groups world-wide. These domain-specific
...
as standalone systems~\cite{SHER-14,GLAT-13}. Most of these frameworks
and instances include one or several workflow engines.
...
workflow engine and instead focus on the execution of simple tasks.
...
high-throughput systems. To this end, science gateways may balance the
load among different instances of the same engine, and use advanced
task scheduling policies on the infrastructure to improve performance,
fault-tolerance and fairness among users.
