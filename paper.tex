...
evaluated on Table~\ref{table:evaluation}. We use 5 main criteria to
...
makes sense. Metric values obtained by each architecture are
normalized between 0 (best value) and 1 (worst value) to determine the
color of the corresponding table cell (brighter color indicate better
performance\footnote{Normalized metric value $m'$ is
  $\frac{m-m_{\mathrm{min}}}{m_{\mathrm{min}}-m_{\mathrm{max}}}$ where
  $m$ is the initial metric value,
  $m_{\mathrm{min}}$/$m_{\mathrm{max}}$ are the minimal/maximal values
  among all architectures. Cell color is \todo{\#99XX99, where
    X=F-4m'})}. The criteria and metrics are explained hereafter.
...
the architecture. It is obtained by counting the number of
interactions and components on the architectural diagram. It breaks
down to the following 2 metrics:
\begin{itemize}[itemsep=0cm]
\item Components \texttt{I$_1$}),
\item Interactions (\texttt{I$_2$}).
\end{itemize}
One may wonder whether infrastructure should be part of
...
why we keep it in the metric. The two metrics are summed to obtain
the total score for this criterion, which measures the total number of
software pieces to develop.

\emph{Robustness of workflow execution} measures the likelihood that
workflow execution fails due to errors coming from the software
architecture. It is determined as the number of software components and
interactions that are specific to workflow execution. It assume that
complex architectures tend to be prone to failure.  These components
and interactions are represented in red on the architecture diagram in
Figure~\ref{fig:architectures}. Robustness breaks down to 2 metrics:
...
\item Interactions (\texttt{R$_2$}): number of specific interactions
  involved in workflow executions.
\end{itemize}
The two metrics are summed to obtain the total score for this
criterion, which measures the total number of software pieces that are
specifically involved in workflow execution. Robustness of workflow
execution is critical since it directly impacts the end users of the
science gateway.

\emph{Extensibility} measures the difficulty to replace or add
elements in the architecture. It is obtained as the number of
