...
infrastructures such as clusters, grids, clouds and web services,
resulting in tools such as Askalon~\cite{fahringer2005askalon},
...
WS-PGRADE/gUSE~\cite{Kacsuk2012}, etc. Such workflow engines usually
...
domain-specific tool repositories, provenance recording, etc.
...
in consistent pipelines. For instance, in neuroimaging, our primary
domain of interest, tools such as Nipype~\cite{gorgolewski2011nipype},
...
and PSOM \todo{...}. Some of these tools also support advanced
features such as provenance tracking (Nipype, PSOM), redundancy
detection across analyses to avoid re-computation (PSOM), and so on. A
wide-array of workflows have been implemented using these pipelines
and are now widely shared across neuroimaging groups
world-wide. \todo{Provide more context on that?} These domain-specific
engines represent a tremendous opportunity for science gateways to
leverage existing tools and applications. They nicely complement
e-Science engines that are more oriented towards the exploitation of
distributed computing infrastructures, in particular grids and clouds.
...
workflow description (a.k.a \emph{workflow}) and input data that may
be files, data base entries, or simple parameter values. Although
...
Several workflow engines may be combined in the same
science gateway.
...
See Figure~\ref{archi:nested}. In nested workflows
(Figure~\ref{archi:nested}-Left), a task of a \emph{parent} workflow
