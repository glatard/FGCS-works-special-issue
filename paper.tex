...
\begin{abstract}
  ...
  scalability and \correction{functionality}. Tight integration and task encapsulation are the easiest
...
Ruby on Rails application\footnote{\correction{Note that the
    ...
    invocation.}}. The workflow engine and the science gateway usually
share a database where applications, users and other resources are
stored.  In this model, task and data control are both initiated from
the science gateway. Interactions \texttt{b} and \texttt{c$_2$} are
initiated from the workflow engine, while \texttt{c$_1$} comes from
other parts of the science gateway, for instance a data management
interface. As in any other model, the installation of new workflows in
the science gateway (\texttt{a$_2$}) and infrastructure
(\texttt{a$_1$}) is done by an administrator, for security
reasons. This is the model adopted in the Catania Science Gateway
...
%\todo{Introduce this section and take the text out of the captions}
...
\includegraphics[width=0.9\textwidth]{figures/VIP}
...
\begin{figure}
  \centering
\subfloat[Nested, Tight+Task]{
  \def\svgwidth{0.5\columnwidth}
  \input{figures/nested-3.pdf_tex}
  \label{fig:nested-3}
}
\subfloat[Nested, Task+Task]{ 
  \def\svgwidth{0.5\columnwidth}
  \input{figures/sub-task.pdf_tex}
  \label{fig:nested-4}
}
