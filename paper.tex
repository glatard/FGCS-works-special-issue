...
\closednote{vladimir}{is this a good word for this context,
  'informed'? maybe 'based on'?}{Tristan}{I guess it is the right
  word, keeping it as is.} by our experience in the development and
...
\closednote{vladimir}{why is it exactly six? are these architectures
  the ones that happened to be implemented during various projects, or
  is it a full set of all possibilities (I believe not)? I think, it
  must be stated clearly why the focus is given to these particular
  ones - we are reviewing and evaluating the approaches that have been
  used already in our projects}{Tristan}{Fixed, thanks.} that cover
...
gateways. \closednote{vladimir}{can we explain why exactly these
  metrics are used but not anything else? I think, we need to justify
  that these ones are especially important for the SG case; it would
  be nice to refer to some related research on software architecture
  metrics, Google gives about 2 million pages on this, where you can
  find metrics like cohesion, depth of scenario and so
  on}{Tristan}{The metrics are specifically designed to cover our
  particular workflow-SG context. Do you have any other
  suggestion?} We finally compare the architectures and highlight
...
WS-VLAM~\cite{wsvlam}. \closednote{vladimir}{As a past developer of it, I
  would suggest to refer to WS-VLAM~\cite{wsvlam} as well, despite it
  is not very famous, but it played an important role for Amsterdam
  e-science workflows}{Tristan}{Done, thanks.} Such workflow engines usually describe
...
\item Workflow integration: consists in adding a new workflow to the
  ...
  the infrastructure or prepared for on-the-fly installation, which
  may require specific privileges; \closednote{vladimir}{or bundled in
    packages considering all needed dependencies than can be deployed
    on the fly during the workflow execution}{Tristan}{added, thanks!} (\texttt{a$_2$}) the
...
workflow engine directly to the user. \closednote{Sil}{about c1: should it
  also show from engine to infra in the figure? this is what i
  understand from the above sentence}{Tristan}{Actually c1 is either
  coming from the science gatewray or from the workflow engine but
  then it merges with c2. I don't think we can cover both cases in a
  single architecture, the latter case would be another variation of
  this architecture. I modified a bit the text to reflect this.}
...
gateway format. \closednote{vladimir}{Are we talking only about importing
  workflows from other wf languages? I would suggest to modify the
  title of the section to 'Workflow conversion and import' then, as
  some gateways use the term 'import' to import their own native
  workflows, like WS-PGRADE, and using import only in the context of
  converting workflows can be confusing}{Tristan}{Good catch, the term
  'import' is actually a bit vague. I guess 'conversion' is more
  accurate. Replaced it everywhere in the text.} Workflow conversion
...
architecture. 
\closednote{vladimir}{I believe, proper error handling through the different levels of wf engine/gateway must be especially mentioned even if not especially covered. One of serious issues we faced while working with gateways at the AMC, in particular with gUse/WS-PGRADE, was that when it failed sometimes it was completely not clear what was the problem exactly. So we might consider discussing not only the number of software components but how explicit and clear interactions are between them; how transparently we can monitor the execution through all the levels and thus detect and prevent possible malfunction}{Tristan}{In our current evaluation framework, this is supposed to be covered by O2. I still agree with you that O2 could be further refined. See Github issue #27}
...
\item Multiple engine instances (\texttt{S$_1$}): measures the
  ...
  \closednote{vladimir}{is this the right word here, 'to
    means'?}{Tristan}{This sentence had been truncated, thanks.} that
...
\item Meta-workflow (\texttt{O$_1$}): measures the ability to describe
  ...
  amd \texttt{O$_1$}=1 means that they may be implemented with some
  development effort. \closednote{si}{why here we do not have the
    "possible to implement" situation?}{Tristan}{O$_1$=2 would never
    be reached. Besides, that would make it difficult to sum O1 and O2
    because they wouldn't measure the same type of thing. In the
    current situation, we are only talking about the presence/absence
    of a particular feature and this is why O1 and O2 can be added to
    obtain a total number of 'missing features'.}
  \closednote{vladimir}{I agree with Silvia, we should have the
    intermediate values as well, since meta-workflows might be
    straightforwardly supported by the gateway, or they are still
    possible to implement but only with some workarounds. Like it was
    with SHIWA: first 6 months coarsed-grained interoperability looked
    more like a workaround till it was better integrated and supported
    by WS-PGRADE itself}{Tristan}{Yes but in the situation you
    describe, evolutions came from the wf engine implementation rather
    than from the architecture to integrate it, which is our point
    here.}
...
\item Fine-grained debugging (\texttt{O$_2$}): availability of
  ...
  interactions. \closednote{vladimir}{otherwise - does it mean the
    fine-grained info is available but hard to access or not available
    it all? the latter happens as well}{Tristan}{Precised the text.}
\end{itemize}
\texttt{O$_1$} and \texttt{O$_2$} are summed to obtain a total number of missing specific
features in the architecture.

\closednote{sil}{since we are dreaming, it seems to me that keeping provenance properly is also something to consider. the architecture can hide some details, and make it nearly impossible to keep provenance without hard coded hacks across the layers/components - i think}{Tristan}{Fixed the definition according to the discussion in PR #20.}
...
\closednote{sil}{why not? i agree that S=2, but i do not agree that it would be impossible. you say how below}{Tristan}{Updated the definition of S$_1$ to explain that multiple engine instances have to be available in the same sg instance.}
(\texttt{S$_1$}=2). 
...
\closednote{sil}{so same as the others = "easy"? i think this requires more explanation. maybe we should introduce a score for "dont know"?}{Tristan}{Fixed as discussed in PR #20}
...
them together.  \closednote{sil}{why not? if the engine is ws-pgrade
  it does. i really think that meta-workflows (and nested workflows)
  is a property exclusively of the engine. you can see that in fig 2.V
  interaction *6}{Tristan}{If the engine is WS-PGRADE as used in SHIWA
  CGI, then the architecture is nested workflows with service
  invocation. In a tight integration, meta-workflows would be
  expressed, e.g. as compositions of Liferay portlets that all
  implement different types of engines.}  The science gateway can
...
specific abilities (\texttt{S$_2$}=1).  \closednote{sil}{is this score for
  VIP? i think that the score depends on the implementation of the
  science gateway, which could adopt some elastic mechanism within. so
  we should not say s_2=1 for the architecture}{Tristan}{Removed VIP example. Updated score definition according to the discussion in @SilviaOlabarriaga's PR (\#20).} The architecture does not add any particular complexity to the task scheduling problem
since the workflow engine may implement any kind of scheduling
policy (\texttt{S$_3$}=0).  \closednote{sil}{see comment in section 3.1}{Tristan}{Fixed the same way}
...
\closednote{sil}{see comment in section 3.1}{Tristan}{same answer} The
science gateway needs to invoke interaction \texttt{d} to retrieve
dine-grained debugging information from the workflow service
(\texttt{O$_2$}=1).  \closednote{sil}{this is really not the case. the
  fact that the wf service is a separate component that hides
  information is a big problem for debugging at the science gateway
  level.  we had a paper about this:
  @inproceedings{olabarriaga2014scientific, Author = {Olabarriaga,
      Silvia and Pierantoni, Gabrielle and Taffoni, Giuliano and
      Sciacca, Eva and Jaghoori, Mahdi and Korkhov, Vladimir and
      Castelli, Giuliano and Vuerli, Claudio and Becciani, Ugo and
      Carley, Eoin and others}, Booktitle = {e-Science (e-Science),
      2014 IEEE 10th International Conference on}, Organization =
    {IEEE}, Pages = {298--305}, Title = {Scientific Workflow
      Management--For Whom?}, Year = {2014}} }{Tristan}{I agree. Fixed
  it and precised the definition of O2.}
...
(\texttt{R$_2$}=1).
\closednote{vladimir}{there is a large piece of text in Scalability about reliability of the workflow engine task which on my opinion suits Robustness better}{Tristan}{I understand that that text may sound like robustness, but the robustness metrics are about the architecture, not the tasks, as explained in 3.1 (Errors coming from
the infrastructure (e.g., unavailable data or termi-
nated tasks) or workflows (e.g., wrong user input or
application errors) are not covered).}
\closednote{vladimir}{in addition, the claim of very good robustness of task encapsulation seems questionable to me as in this model you delegate the critical task of controlling workflow execution to, let's say, some 'random' node on the grid that you have no control of, not sure about its performance, connectivity and reliability and so on; however, considering the models and dependencies presented in fig.2 this evaluation of robustness metric may be correct}{Tristan}{You are right but the robustness issues that you are talking about are related to the infrastructure and not the architecture. This is really an issue related to task scheduling as the SG might decide to run the workflow tasks on reliable nodes of the infrastructure such as a dedicated cluster. So the issue that you mention is reflected in S3.}
...
which increases its chances of failure. \closednote{vladimir}{isn't this about robustness?}{Tristan}{Nope, this is task scheduling, therefore scalability in our evaluation framework. See answer to your previous note.} In addition, task parameters,
...
development. \closednote{sil}{see comment in section
  3.1}{Tristan}{Fixed, actually updated the score of this architecture
  now that the definition is more accurate.} Fine-grained debugging
information is obtained through interaction \texttt{b}
(\texttt{O$_2$}=1). \closednote{sil}{mm, i think the case here is the
  same as for service. or actually i would give this one a better
  score, because there are less components to cross}{Tristan}{Fixed the text to better match the updated definition of O2}
...
new agents, which can be easily automated (\texttt{S$_1$=0}). For
instance, auto-scaling rules can be implemented to start new agents
when the workload in the science gateway exceeds a certain
threshold~\cite{lorido2012auto}. Distributed engines are not available
...
(\texttt{O$_1$=1}).  \closednote{sil}{see comment in section 3.1}{Tristan}{Same answer.} Debugging information is accessed through interactions \texttt{d} and \texttt{f} (\texttt{O$_2$=1}).
\closednote{sil}{i think the case here is the same as for service. or
  actually i would give this one a worse score, because there is one
  additional component - the pool - to cross}{Tristan}{Fixed using the updated definition of O2}
...
\texttt{d} (twice).
\closednote{vladimir}{and if we have a meta-workflow with 3 and more sub-workflows the score will be even higher, is it a fair calculation for an architecture? we did not increase the scores for possible meta-workflows supported by wf engines in other cases}{Tristan}{Good point! You are totally right for meta-workflows, I added a sentence in the paragraph below. However, nested workflows are studied here as an architecture to integrate a workflow engine (and only one), not to describe meta-workflows.}
...
child workflows executed by 2 different engine types would require an
...
science gateway (\texttt{O$_1$=0}).  \closednote{sil}{i dont
  understand this about meta-workflows. translation does not imply
  composition, right?  if the science gateway is allowed to implement
  workflow composition, then this is probably the case in other
  architectures as well?}{Tristan}{Well at this level, connecting two
  workflows expressed in the same language is straightforward,
  although I agree that it might be tricky in some cases.} Debugging information is accessible
as in the service invocation architecture (\texttt{O$_2$=1}).
\closednote{sil}{see notes 3.2}{Tristan}{Fixed the same way}
...
encapsulation and pool. \closednote{Naj}{I feel this paragraph is a bit
  vague, and doesn't distill and contextualize the thorough work
  described before.}{Tristan}{Rejected comment :)}
...
invocation) are the only architectures that intrinsically support meta-workflows. \closednote{vladimir}{As mentioned earlier by Silvia, this might not be exactly the case, depending on the point of view}{Tristan}{The example given by Silvia (WS-PGRADE) actually comes down to the nested workflow architecture. I added 'intrinsically' to the sentence to better reflect that.} Besides,
tight integration is the only architecture that allows
fine-grained debugging. 
The availability of fine-grained debugging information may be
...
(service invocation), and may challenge reliability more strongly. However, without entering into the details of a particular
...
instantiations.\closednote{Naj}{I think in the discussion, you may want to close the circle on a statement in introduction, which referred to your 7-year experience with CBRAIN and VIP}{Tristan}{Not sure how to do this without making the conclusion sound too specific to these systems.}
...
already available as web services\closednote{Naj}{This is a good place to offer some examples}{Tristan}{Many of them are, precised the text, not sure adding explicit examples makes sense at this stage.}, which tends to favor service
...
\closednote{vladimir}{@silvia, does it make sense to mention workflow classification efforts based on workflow motifs and so on?}{Tristan}{I'm  not  sure. Which papers are you thinking of?}

\section{Conclusion}

% Summary of contributions
We have systematically reviewed architectures used to integrate
workflow engines in science gateways. These architectures are
