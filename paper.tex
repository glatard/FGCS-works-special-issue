...
\begin{abstract}
	\note{sil}{i find this a dry start. what about an intro sentence?
		...
		and services offered by science gateways.}
  We investigate six software architectures commonly used to integrate
  workflow engines into science gateways. In \emph{tight integration},
  ...
  metrics for assessment of integration complexity, robustness,
  ...
  science gateway architects and workflow engine developers. 
  \note{sil}{what are the insights for wf engine developers?. this is not clear in the discussion and conclusions. i think the focus lies more on the integration and not on one or the other} Tight
  integration and task encapsulation are the easiest to integrate and
  the most robust. Extensibility is equivalent in most
...
\note{sil{what about closing with this sentence from above:
...
\closednote{Sil}{i changed above a few occurrences of "diagrammed". i know it is correct, but i am not used to seeing it. this is a matter of style, just forget my changes below then}{Tristan}{That's perfect, thanks!}
...
\closednote{Sil}{is this next sentence relevant?}{Tristan}{I just wanted to  justify why these two interactions can be grouped but I guess you're right (I removed the sentence.}%In practice, both
...
other specific features. These are based on science gateway properties introduced in Section~\ref{sec:intro_sg}. Criteria break down to specific metrics where
\emph{lower value indicates better performance}. For each criterion, a
global score is computed by summing up the individual scores for each metric. We
ensure that the different metrics used to calculate the global score for a criterion are comparable, so that they can be combined meaningfully. The criteria and metrics are
explained hereafter.

\note{Sil}{normalization is explained in caption tab 2. i wonder whether something should be said here, as questions about the range of the score immediately pops to mind..}
...
\note{Sil}{i was wondering whether it would be more clear to present the metrics like this:
\item \texttt{I$_1$}: total number of components, and

now the parenthesis are next to text that does not really match the metric definition (exception in this case) could also use boldface
	}
...
\begin{itemize}[leftmargin=0cm,itemindent=0.35cm,itemsep=0cm]
\item Number of specific components
involved in workflow executions (\texttt{R$_1$}). The Science
...
user data, etc., and
\item Number of specific interactions
involved in workflow executions (\texttt{R$_2$}).
\note{Sil}{add here an example of an interaction that is not, for example, data transfers?}
\end{itemize}
...
\item New engine \texttt{E$_1$}: number of interactions or
  components that need to be modified to integrate a new type of workflow engine in
  the architecture. Workflow engines belong to different types when
  they cannot be invoked using the same interface. 
  \note{Si}{new interface = new interaction? maybe better to use one term only here. Maybe also refer to how this shows on figure 2}
\item Engine version upgrade \texttt{E$_2$}: number of interactions or
  components modified to integrate a new version of a workflow engine
  ...
  \note{Silv}{it seems to me that this metric cannot be calculated in a straighforward manner from the diagrams.. is that correct? if it can, give an example of how this is seen in the diagram}
\item New workflow \texttt{E$_3$}: number of interactions required
to integrate a new workflow in the architecture. Adding a new workflow is a very
  common operation that does not require modifying software components
  or interactions, assuming
  that the engine type and version required to execute this workflow
  are already available.
  \note{sil}{again here it would be good to indicate how this is seen in the diagram}
...
\begin{itemize}[leftmargin=0cm,itemindent=0.35cm,itemsep=0cm]
\item Multiple engine instances (\texttt{S$_1$}): measures the possibility \note{sil}{are we measuring possibility or potential? i think the second, because it is hypothetical. possibility brings me the idea of it is possible or not - which would be zero to almost all these features} of having
  ...
  intervention (e.g., two-factor authentication \note{sil}{what is this?}), which prevents easy
  implementations of elastic engines. \texttt{S$_1$}=2 means that a new engine
  instances cannot be added.
\item Distributed engines (\texttt{S$_2$}): measures the possibility of distributing
  ...
  engine. \texttt{S$_2$}=0 means that distributed engines are possible in
...
\item Task scheduling (\texttt{S$_3$}): measures ??? \note{sil}{need to be more specific about what is being called task scheduling here. also explain why it is difficult, or dont say anything if it is common knowledge}. Task scheduling typically depends more on the implementation of specific algorithms
  ...
  creating tasks with specific characteristics. \texttt{S$_3$}=0 means that
...
\item Meta-workflow (\texttt{O$_1$}): measures the ability to describe
  ...
  components. \texttt{O$_1$}=0 means that the feature is available, and
...
\closedanswerednote{Tristan}{What-about load-balancing between web
...
\paragraph{Scalability} The service architecture in principle supports multiple
...
executions to the engine instance that has the least active
executions. To avoid ``black-hole'' syndromes created by failing
 instances, engine instances are automatically disabled when
...
\note{sil}{is this score for VIP? i think that the score depends on the implementation of the science gateway, which could adopt some elastic mechanism within. so we should not say s_2=1 for the architecture}
The scheduling of tasks on the
...
invocation) are the only architectures that support meta-workflows. Besides,
...
system \closednote{Naj}{so are you discussing it here, or in the
  future?}{Tristan}{It's out of scope, a complete paper in itself. Rephrased this part.} since this interaction may not be easily generalizable to any
...
so on.
\note{sil}{do you want to refer to the science gateway canvas paper here?
...
\note{sil}{should we then also thank the dutch crowd?
	 ...
	 We are also greatful to the financial support provided by FP7 E-INFRASTRUCTURE program for financial support to SCI_BUS, SHIWA and ER-flow projects.  
	 My participation has been minimal, and the amc gateways are not discussed in detail (my fault), so maybe it would be an overkill}
